...
//                        p.$jot('.jt-content, .jt-title').css('width', 'auto');
   ...
   */
    ...
    isInputAlwaysHidden: function (input) {
        ...
        if (collapseSection && (collapseSection.hasClassName('always-hidden') || collapseSection.hasClassName('form-field-hidden'))) {
            return true;
        }
    ...
    isSectionTouched: function (section) {
        ...
        for (var i = 0; i < inputs.length; i++) {
            ...
            if (fieldType === 'control_rating') {
            ...
            } else if (fieldType === 'control_payment' || fieldType === 'control_' + JotForm.payment) {
                var productInputs = field.querySelectorAll('input.form-product-input');
                var productInputsFiltered = productInputs.filter(function (el) {
                    if (isDefaultValue(el)) {
                        return false;
                    }
                    return el.checked;
                ...
                if (productInputsFiltered.length > 0) {
                    filled++;
                }
